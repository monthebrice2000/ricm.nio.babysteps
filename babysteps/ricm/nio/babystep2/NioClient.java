...
public class NioClient {
	...
	private void handleConnect(SelectionKey key) throws IOException {
		...
		/*byte[] tmp = new byte[ 2 * this.first.length ];
		System.arraycopy(this.first, 0, tmp, 0, this.first.length );
		System.arraycopy(this.first, 0, tmp, this.first.length , this.first.length );
		digest = md5(tmp);
	...
	private void handleWrite(SelectionKey key) throws IOException {
		...
		WriterAutomata wa = new WriterAutomata( sc, selector );
		wa.handleWrite( outBuffer );
	...
	public static void main(String args[]) throws IOException {
        ...
        lengthbytes = ( String.valueOf( msg.length() ) ).getBytes( Charset.forName("UTF-8"));
        // combine message length and message an multidimensional array
		byte[] bytes = new byte[ 4 + msgbytes.length ];
		System.arraycopy( lengthbytes, 0, bytes, 0, lengthbytes.length);
		System.arraycopy( msgbytes, 0, bytes, 4, msgbytes.length);
